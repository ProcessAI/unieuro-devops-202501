...
export function HeroCarousel() {
    ...
    const buscarImagensCarrossel = async () => {
      try {
        ...
        if (!response.ok) {
          ...
          throw new Error(
            `Erro ao buscar dados do carrossel: ${response.status}. Resposta: ${errorText.substring(0, 200)}...`
          );
        }
  ...
  if (error) {
    return (
      <div className="text-red-500 text-center py-20">Erro ao carregar os destaques: {error}</div>
    );
  }
          ...
          >
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center flex-col md:flex-row">
              <div className="text-left md:w-1/2">
                ...
                <span className="cursor-pointer px-6 py-3 bg-[#DF9829] text-white rounded-lg hover:bg-[#C77714] transition">
                  Ver detalhes →
                </span>
              </div>
